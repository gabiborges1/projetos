...
1. Crie uma variável na base de dados my_df com o CID de três digítos CID3dig. Em seguida, crie outra variável ind_tuberculosis para indicar se o óbito foi por causa de tuberculosis (CIDs A10 a A16).  PS: Utilize as funções do dplyr e lembre-se de guardar o resultado utilizando o operador de assignment.

```{r}
my_df <- my_df %>% 
  mutate(CID3dig = substr(CAUSABAS, start = 1, stop = 3),
         ind_tuberculosis = CID3dig %in% c("A10", "A11", "A12", "A13", "A14", "A15", "A16")
         )
```
...
obitos_mun <- my_df %>% 
  ...
  summarise(
    n_obitos = n(),
    n_obitos_tb = sum(ind_tuberculosis),
    n_obitos_mulheres = sum(SEXO == "Feminino", na.rm = T),
    prop_mulheres = n_obitos_mulheres/n_obitos*100,
    n_obitos_menos5 = sum(IDADEanos <= 5, na.rm = T)
  )
```
...
```{r}
df_cities <- my_df_cities %>% 
  select(CITY, CD_GEOCMU, starts_with("IBGE"), starts_with("IDHM")) %>% 
  filter(!is.na(CD_GEOCMU))
```
...
```{r}
df_cities <- df_cities %>% 
  mutate(CD_GEOCMU = as.numeric(substr(CD_GEOCMU, 1, 6)))
```
...
```{r}
dados_final <- right_join(
    x = df_cities, y = obitos_mun, by = c('CD_GEOCMU' = 'CODMUNRES')
  )

write_csv(dados_final, "C://Users/gabil/Documents/curso R/data/dados_joined.csv")
```
